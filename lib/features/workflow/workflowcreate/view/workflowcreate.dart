...
class WorkflowCreate extends StatefulWidget {
  ...
  WorkflowCreate({required this.datas, required this.isEdit, this.datas_onEdit});
...
class _WorkflowCreateState extends State<WorkflowCreate> {
          ...
          AppBarAction(
              ...
              onPressed: () => {
                    if (form_field_Values?.length == 0)
                      {Snack.errorSnack(context, Strings.alert_error_invalidform)}
                    else
                      {
  ...
  Widget loadWidgets(int iPanel, int indexwidget, var widgetData, bool isEdit, var datas_onEdit) {
    Widget child;
    ...
    switch (widgetType) {
      case Strings.label:
        child = Container(
            child: Labels(
                sLabel: widget.datas.settings.specific.tableColumns[indexwidget].label,
                bRequired: false));
        break;
      case Strings.divider:
        child = Dividers(
            sType: widget
                .datas.settings.specific.tableColumns[indexwidget].settings.general.dividerType);
        break;
      case Strings.shortText:
        child = TextInput(
          borderColor: CustomColors.grey,
          initialValue: (isEdit) ? findValueUsingId(datas_onEdit, widgetData.id) : "",
          label: widget.datas.settings.specific.tableColumns[indexwidget].label,
          hasError: controller.hasTextError.value,
          icon: null,
          keyboardType: getInputType(widget
              .datas.settings.specific.tableColumns[indexwidget].settings.validation.contentRule),
          onChanged: (p0) {
            updateFormValues_WhileSubmit(
                widget.datas.settings.specific.tableColumns[indexwidget], p0, widget.datas.id);
          },
          bOptional: widget.datas.settings.specific.tableColumns[indexwidget].settings.validation
                  .fieldRule ==
              Strings.required,
          placeholder: '',
        );
      ...
      case Strings.date:
        child = DateTimeInput(
          ...
          onChangedDate: (p0) {
            dateValue = p0;
            updateFormValues_WhileSubmit(widget.datas.settings.specific.tableColumns[indexwidget],
                (timeValue.isNotEmpty) ? dateValue + "," + timeValue : dateValue, widget.datas.id);
          },
          onChangedTime: (p0) {
            timeValue = p0;
            updateFormValues_WhileSubmit(widget.datas.settings.specific.tableColumns[indexwidget],
                (dateValue.isNotEmpty) ? dateValue + "," + timeValue : timeValue, widget.datas.id);
          },
          placeholder:
              widget.datas.settings.specific.tableColumns[indexwidget].settings.general.placeholder,
        );
        break;
      ...
      case Strings.multipleSelect:
        child = CheckBoxInput(
            ...
            options: widget
                .datas.settings.specific.tableColumns[indexwidget].settings.specific.customOptions
                .toString()
                .split(','),
            slabel: widget.datas.settings.specific.tableColumns[indexwidget].label,
            bOptional: widget.datas.settings.specific.tableColumns[indexwidget].settingsvalidation
                    .fieldRule !=
                Strings.required,
  ...
  Widget DropdownOptions(int iPanel, int indexwidget) {
    ...
    if (widget.datas.settings.specific.tableColumns[indexwidget].settings.specific.optionsType ==
        Strings.custom) {
      child = DropDownMain(
        borderColor: CustomColors.grey,
        initialValue: '',
        label: widget.datas.settings.specific.tableColumns[indexwidget].label,
        sColumnName: widget.datas.settings.specific.tableColumns[indexwidget].id,
        hasError: controller.hasNumberError.value,
        icon: null,
        keyboardType: getInputType(widget
            .datas.settings.specific.tableColumns[indexwidget].settings.validation.contentRule),
        ...
        bOptional: widget
                .datas.settings.specific.tableColumns[indexwidget].settings.validation.fieldRule ==
            Strings.required,
        placeholder: '',
        sInputs: widget
            .datas.settings.specific.tableColumns[indexwidget].settings.specific.customOptions,
        sInputSeperator: widget.datas.settings.specific.tableColumns[indexwidget].settings.specific
            .separateOptionsUsing,
        sDropDownType:
            widget.datas.settings.specific.tableColumns[indexwidget].settings.specific.optionsType,
      );
    }

    if (widget.datas.settings.specific.tableColumns[indexwidget].settings.specific.optionsType ==
            Strings.existing &&
        !widget.datas.settings.specific.tableColumns[indexwidget].settings.specific
            .allowToAddNewOptions) {
      controller.getDropDownValues(controller.sFormId,
          widget.datas.settings.specific.tableColumns[indexwidget].id, iPanel, indexwidget, false);
      child = DropDownMainAPI(
          ...
          sInputSeperator: widget.datas.settings.specific.tableColumns[indexwidget].settings
              .specific.separateOptionsUsing,
          sDropDownType: widget
              .datas.settings.specific.tableColumns[indexwidget].settings.specific.optionsType);
    }
  ...
  Future<void> updateFormValues_WhileSubmit([var data, var input, final parentId]) async {
