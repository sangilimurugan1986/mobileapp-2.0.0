...
import 'package:ez/core/v5/models/popup/form/components/reset_text_field_date.dart';
...
class _DateTimeInputState extends State<DateTimeInput> {
  ...
  TimeOfDay time12to24Format(String time) {
    String dateTimeSplit = (time.split(",").length > 1) ? time.split(",")[1] : time;
    int h = int.parse(dateTimeSplit.split(":").first);
    int m = int.parse(dateTimeSplit.split(":").last.split(" ").first);
    String meridium = dateTimeSplit.split(":").last.split(" ").last.toLowerCase();
    if (meridium == "pm") {
      if (h != 12) {
        h = h + 12;
      }
  ...
  Future _showTimePicker(BuildContext context) async {
    final time = await showTimePicker(
      context: context,
      initialTime: (modelValueTime == '') ? TimeOfDay.now() : time12to24Format(modelValueTime),
    );
      ...
      children: [
        ...
        Expanded(
          child: ResetTextFieldDate(
            ...
            placeholder: widget.placeholder.length > 0 ? widget.placeholder : 'YYYY-MM-DD',
          ),
        ),
        ...
        Expanded(
          child: ResetTextFieldDate(
            ...
            placeholder: widget.placeholder.length > 0 ? widget.placeholder : 'HH:MM',
          ),
        ),
        // show date picker btn
        GestureDetector(
